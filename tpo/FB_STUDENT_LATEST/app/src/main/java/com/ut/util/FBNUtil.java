...
import com.ut.fbn.ErrorActivity;
...
import com.ut.receiver.EventReceiver;
import com.ut.receiver.FeeReminderReceiver;
import com.ut.receiver.NoticeReceiver;
...
public class FBNUtil {
	...
	public static final String MY_PREFS_NAME = "Fresher Buddy";
	...
	public static void showNoticeNotification(Context context, int size, Intent intent) {
		PendingIntent pIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_IMMUTABLE);
		// Create an intent for the reply action
		Intent actionIntent = new Intent(context, NoticeActivity.class);
		...
		Notification.Action action = new Notification.Action.Builder(
				R.drawable.icon_login_tab, context.getString(R.string.Show_List), actionPendingIntent).build();

		Notification notification = new Notification.Builder(context, CHANNEL_ID)
				.setTicker(context.getString(R.string.New_Notice))
				.setContentTitle(size + " " + context.getString(R.string.New_Notice))
				.setContentText(size + " " + context.getString(R.string.New_Notice))
		...
		Toast.makeText(context, context.getString(R.string.New_Notice), Toast.LENGTH_LONG).show();
	}

	public static void showFeeNotification(Context context, int size, Intent intent) {
		PendingIntent pIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_IMMUTABLE);
		// Create an intent for the reply action
		Intent actionIntent = new Intent(context, FeeActivity.class);
		PendingIntent actionPendingIntent = PendingIntent.getActivity(context, 0, actionIntent,
				PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
		...
		Notification notification = new Notification.Builder(context, CHANNEL_ID)
				.setTicker(context.getString(R.string.New_FEE_REMINDER))
				.setContentTitle(size + " " + context.getString(R.string.New_FEE_REMINDER))
				.setContentText(size + " " + context.getString(R.string.New_FEE_REMINDER))
				.setSmallIcon(R.drawable.ic_launcher).setContentIntent(pIntent)
				.addAction(action)
				.build();
		NotificationManager mNotificationManager = (NotificationManager) context
		...
		Toast.makeText(context, context.getString(R.string.New_FEE_REMINDER), Toast.LENGTH_LONG).show();
	}

	public static void showNotification(Context context, int size, Intent intent) {
		PendingIntent pIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_IMMUTABLE);
		// Create an intent for the reply action
		Intent actionIntent = new Intent(context, EventActivity.class);
		PendingIntent actionPendingIntent = PendingIntent.getActivity(context, 0, actionIntent,
				PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
		...
		Notification notification = new Notification.Builder(context, CHANNEL_ID)
				.setTicker(context.getString(R.string.New_Opening))
				.setContentTitle(size + " " + context.getString(R.string.New_Opening))
				.setContentText(size + " " + context.getString(R.string.New_Opening))
				.setSmallIcon(R.drawable.ic_launcher).setContentIntent(pIntent)
				.addAction(action)
				.build();
		NotificationManager mNotificationManager = (NotificationManager) context
		...
		Toast.makeText(context, context.getString(R.string.New_Opening), Toast.LENGTH_LONG).show();
	}
	...
	public static EligibleHallTicket convertEligibleEventRequest(JSONObject obj) throws JSONException {
		EligibleHallTicket eligibleHallticket = new EligibleHallTicket();
	...
	public static Notice convertNoticeRequest(JSONObject obj) throws JSONException {
		...
		notice.setFileName1(obj.optString("fileName1", null));
		notice.setFileName2(obj.optString("fileName2", null));
		notice.setFileName3(obj.optString("fileName3", null));
		notice.setFileName4(obj.optString("fileName4", null));
		notice.setFileName5(obj.optString("fileName5", null));
		notice.setFile1(obj.optString("file1", null));
		notice.setFile2(obj.optString("file2", null));
		notice.setFile3(obj.optString("file3", null));
		notice.setFile4(obj.optString("file4", null));
		notice.setFile5(obj.optString("file5", null));
		return notice;
	}

	public static Result convertResultRequest(JSONObject obj) throws JSONException {
		Result result = new Result();
		result.setLoginname(obj.getString("enrollmentNo"));
		result.setAttempt(obj.getString("attempt"));
		result.setTotalnumbers(obj.getString("totalnumbers"));
		result.setResult(obj.getString("result"));
		result.setTestName(obj.getString("testName"));
		result.setDateTaken(obj.getString("dateTaken"));
		result.setTotalTimeTaken(obj.getString("totalTimeTaken"));
		result.setNumberOfQuestion(obj.getString("numberOfQuestion"));
		result.setTotalTime(obj.getString("totalTime"));
		result.setCreatedBy(stripHtml(obj.getString("createdBy")));
		result.setCertificateAvialable(obj.getBoolean("certificateAvialable"));
		result.setCertificate(Base64.decode(obj.getString("certificate")));
		return result;
	}

	public static FeeReminder convertFeeRequest(JSONObject obj) throws JSONException {
		FeeReminder feeReminder = new FeeReminder();
		feeReminder.setId(obj.getInt("id"));
		feeReminder.setAmountPaid(obj.getDouble("amountPaid"));
		feeReminder.setPaidOn(obj.getString("paidOn"));
		feeReminder.setAmountDue(obj.getDouble("amountDue"));
		feeReminder.setDueOn(obj.getString("dueOn"));
		return feeReminder;
	}

	public static Document convertDocumentRequest(JSONObject obj) throws JSONException {
		Document document = new Document();
		document.setDocumentName(obj.getString("documentName"));
		document.setUploadedDate(obj.getString("uploadedDate"));
		document.setVerifiedWithOrgnal(obj.getBoolean("isVerifiedWithOrgnal"));
		document.setVerifiedDate(obj.getString("verifiedDate"));
	...
	public static void showWSErrorDetails(Context context) {
		try {
			...
			msg.setText(Html.fromHtml(
					"<font color='red'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Service not available</font>", Html.FROM_HTML_MODE_LEGACY));
			// set dialog message
			alertDialogBuilder.setView(msg).setCancelable(false).setPositiveButton("Close",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
	...
	public static void showInternetErrorDetails(Context context) {
		...
		TextView msg = new TextView(context);
		msg.setText(
				Html.fromHtml("<font color='red'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Internet not available</font>", Html.FROM_HTML_MODE_LEGACY));
		// set dialog message
		alertDialogBuilder.setView(msg).setCancelable(false).setPositiveButton("Close",
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
	...
	public static boolean isUserLoggedIn(Context context) {
		SharedPreferences prefs = context.getSharedPreferences(FBNUtil.MY_PREFS_NAME, Context.MODE_PRIVATE);
		if (prefs != null) {
	...
	public static void scheduleEventRecever(Context context, Context baseContext) {
		Intent intentAlarm = new Intent(context, EventReceiver.class);
	...
	public static void scheduleFeeReminderRecever(Context context, Context baseContext) {
		Intent intentAlarm = new Intent(context, FeeReminderReceiver.class);
		setReceiver(context, baseContext, intentAlarm);
	...
	private static void setReceiver(Context context, Context baseContext, Intent intentAlarm) {
		...
		alarmManager.setRepeating(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime(),
				(long) FBNConstants.NOTIFICATION_TIME_IN_MINUTE * 60 * 60 * 1000, pendingIntent);
	}
	...
	public static void setLogin(String userName, Context context) {
		SharedPreferences sharedPreferences = context.getSharedPreferences(FBNUtil.MY_PREFS_NAME, Context.MODE_PRIVATE);
	...
	public static String callWebService(JSONObject dato, String wsUrl){
