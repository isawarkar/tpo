...
$ApplicationLoadBalancerName = "fb-ecs-load-balancer"
$targetGroupName = "$clusterName-target-group"
$healthCheckPath = "/$alias/actuator/health"
...
function getALBListnetARN {
	# Variables
$albArn = aws elbv2 describe-load-balancers --names $ApplicationLoadBalancerName --region $region --query "LoadBalancers[0].LoadBalancerArn" --output text

# Get all listeners for the ALB
$listeners = aws elbv2 describe-listeners --load-balancer-arn $albArn | ConvertFrom-Json

# Filter for HTTPS listener (port 443)
$httpsListener = $listeners.Listeners | Where-Object { $_.Port -eq 443 -and $_.Protocol -eq "HTTPS" }

# Extract the listener ARN
$listenerArn = $httpsListener.ListenerArn

# Output
Write-Host "✅ HTTPS Listener ARN: $listenerArn"
return $listenerArn
}
...
$listenerArn = getALBListnetARN
if ($result -eq "Yes") {
    deleteECSCluster -listenerArn $listenerArn
	createECSCluster -listenerArn $listenerArn
} else {
    deleteECSCluster -listenerArn $listenerArn
	}
}else {
