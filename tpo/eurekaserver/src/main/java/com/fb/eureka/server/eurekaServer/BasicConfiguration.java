/*
 * package com.fb.eureka.server.eurekaServer;
 * 
 * 
 ...
 * @Configuration
 * 
 * @EnableWebSecurity public class BasicConfiguration {
 * 
 * @Bean public InMemoryUserDetailsManager userDetailsService(PasswordEncoder
 * passwordEncoder) { UserDetails admin = User.withUsername("indrajeet")
 * .password(passwordEncoder.encode("teejardni")) .roles("USER", "ADMIN")
 * .build();
 * 
 * return new InMemoryUserDetailsManager(admin); }
 * 
 * @Bean public SecurityFilterChain filterChain(HttpSecurity http) throws
 * Exception { http.csrf().disable().authorizeRequests() .anyRequest()
